...
Button.prototype.tick = function()
{
    if(this.item.itemHandler.windowHandler.isMouseDown())
    {
        ...
        {
            if(mousesdown[i].x >= this.alignx.sx() + this.x && mousesdown[i].y >= this.alignx.sy() + this.y && mousesdown[i].x < this.alignx.sx() + this.x + this.width && mousesdown[i].y < this.alignx.sy() + this.y + this.height)
            {
                this.onClick();
                break;
            }
